...
def generate_image(width, height, min_depth, max_depth):
    ...
    pixels = image.load()
    channels = tuple(build_random_function(min_depth, max_depth)
                     for _ in range(3))
    for i in range(width):
        for j in range(height):
            ...
            pixels[i, j] = tuple(color_map(function(x, y))
                                 for function in channels)
    return image
...
def get_parser(name):
    ...
    parser.add_argument("--width", type=int, default=DEFAULT_WIDTH,
                        help="Width of the generated image in pixels (default:"
                             f" {DEFAULT_WIDTH})")
    parser.add_argument("--height", type=int, default=DEFAULT_HEIGHT,
                        help="Height of the generated image in pixels (default:"
                             f" {DEFAULT_HEIGHT})")
    parser.add_argument("--min-depth", type=int, default=DEFAULT_MIN_DEPTH,
                        help="Minimum recursive depth of color functions"
                             f" (default: {DEFAULT_MIN_DEPTH})")
    parser.add_argument("--max-depth", type=int, default=DEFAULT_MAX_DEPTH,
                        help="Maximum recursive depth of color functions"
...
def main(args):
    ...
    image = generate_image(parsed_args.width, parsed_args.height,
                           parsed_args.min_depth, parsed_args.max_depth)
